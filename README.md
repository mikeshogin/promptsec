...
Security coaching for AI prompts. Age-appropriate ratings (6+, 12+, 16+, 18+), topic guidance, safety classification. No LLM required.

## How It Works

```
prompt -> promptsec (classify) -> rating + flags
...
```bash
go install github.com/mikeshogin/promptsec/cmd/promptsec@latest
```
...
```bash
# Rate a prompt
echo "Help me write a school essay about history" | promptsec rate
# Output: {"rating": "6+", "safe": true, "flags": []}

# Check if prompt passes a threshold
echo "Explain SQL injection attacks" | promptsec check --max-rating 12
# Exit 1 (rating is 16+, exceeds threshold)

# Filter mode - block or sanitize
echo "Some prompt text" | promptsec filter --policy strict
```
...
```
prompt -> promptsec (safe?) -> promptlint (route) -> agent -> archlint (quality)
              |                                                       |
...
echo "prompt" | promptsec check --max-rating 16 && echo "prompt" | promptlint analyze
```

### As ccproxy hook

promptsec can run as a pre-routing hook in ccproxy - block or flag prompts before they reach any model.
